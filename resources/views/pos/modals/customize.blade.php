@if ($this->customizingFood)
    <div class="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4">
        {{-- A dish with many ingredients plus the note field runs past the
             bottom of a 10" screen, and what goes first is the row with Salva.
             So the dialog is capped and the ingredients scroll inside it: the
             title and the two buttons are always on screen. --}}
        <div class="flex max-h-[85vh] w-full max-w-md flex-col rounded-2xl bg-white p-6 shadow-xl dark:bg-neutral-900">
            <h2 class="shrink-0 text-xl font-semibold">{{ $this->customizingFood->name }}</h2>

            <div class="min-h-0 flex-1 overflow-y-auto overscroll-contain">
                @if ($this->customizableIngredients->isNotEmpty())
                    <div class="mt-4 space-y-2">
                        @foreach ($this->customizableIngredients as $ingredient)
                            <div wire:key="ing-{{ $ingredient->id }}" class="flex items-center justify-between rounded-lg border border-neutral-200 p-2 dark:border-neutral-800">
                                <div class="min-w-0">
